...
test("interpolate resolves query path with suffix in moustache", () => {
  const out = interpolate("Image: {{ $item.picture?w=48 }}", {
    globals: {},
...
test("interpolate fallback works with query paths", () => {
  const out = interpolate("Avatar: {{ $profile.picture || $profile.avatar }}", {
    globals: {},
